...
func (g *generator) generateServiceClient(file *descriptorpb.FileDescriptorProto, service *descriptorpb.ServiceDescriptorProto) *pluginpb.CodeGeneratorResponse_File {
    ...
    g.P(`      int retries`)
    g.P(`  ){`)
    g.P(`    final java.util.concurrent.atomic.AtomicInteger count = new java.util.concurrent.atomic.AtomicInteger(retries);`)
    g.P(`    return java.util.concurrent.Executors.newSingleThreadExecutor().submit(() -> {`)
    g.P(`      R result = null;`)
    g.P(`      while(count.getAndDecrement()>0){`)
    g.P(`        try{`)
    g.P(`          result = call(path, request, headers, responseClass);`)
    g.P(`          if(predicate.apply(result)){`)
    g.P(`            return result;`)
    g.P(`          }`)
    g.P(`          Thread.sleep(retryTimeout);`)
    g.P(`        } catch(javax.ws.rs.WebApplicationException ex){`)
    g.P(`          if(count.get()<=0){throw ex;}`)
    g.P(`        }`)
    g.P(`      }`)
    g.P(`      return result;`)
    g.P(`    });`)
